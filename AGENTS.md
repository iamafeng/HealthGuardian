...
**Pet module** (`Pet` in `app.js`): sprite-based pixel cat with 4×4 sprite sheet (4 states × 4 frames). States: `idle` (row 0), `walk` (row 1), `play` (row 2), `sleep` (row 3). Animation runs at 200 ms/frame via `setInterval`. `Pet.update(drinkToday, restToday)` sets HP and mood text based on total daily check-ins. `Pet.setBadPosture(true)` forces `sleep` state and shows a warning speech bubble. Click interaction (`Pet.interact()`) plays `meow.wav`, triggers `play` state for 2 s, and shows random speech. After 8+ clicks, the pet gets "angry" for 5 s. Hover triggers `play` state for 1 s. Pet feeding (via the pet UI button) calls `App.completeTask('DRINK', null, true)` which also calls `API.post('/api/pet/feed', ...)`.
...
**Pet vitals system**: `Pet.updateVitals(drinkToday, restToday)` computes hunger (20 + drinkToday×15, max 100), energy (20 + restToday×20, max 100), clean (decreases by 5 per frame when bad posture, increases by 2 otherwise). All three render to progress bars in `#pet-modal`.

**widget.html v2 (Pet Sprite)**: Replaced data card with cat sprite (96×96, 4-state animation). Collapsed state: sprite + hp bar only (140×160). Hover: expands data panel (220×280) via `widget-resize` IPC. Left click: `openMain()`. Right click: `petInteract()` (meow + play animation + speech bubble). Bad posture: sprite gets red filter + sleep state.
