...
这是一个融合了 **Java Spring Boot 后端**、**GuardianDesktop 桌面壳层**与 **TensorFlow.js 计算机视觉**的全维度健康守护平台。它不仅仅是一个提醒工具，更是一个通过 AI 技术实时监测用户状态的智能健康底座。
...
> **⚠️ Windows SmartScreen 安全提示**
> 首次运行下载的 `.exe` 时，Windows 可能弹出"Windows 已保护你的电脑"蓝色警告。这是因为本项目为开源免费软件，尚未购买商业代码签名证书（EV 证书约 $300-500/年），属于开源软件的常见情况。
> **处理方式**：点击 **「更多信息」** → **「仍要运行」** 即可正常安装。
> 本项目代码完全开源，可在 [GitHub 仓库](https://github.com/iamafeng/HealthGuardian) 审查全部源码。
...
3.  **桌面端 (GuardianDesktop)**: 作为跨平台外壳，提供系统托盘、全局快捷键、窗口管理及本地服务自启动/连接管理。
...
*   **AI/CV 引擎**: TensorFlow.js (MoveNet 模型，实现零延迟本地姿态识别，依赖文件已本地化)
*   **桌面外壳**: GuardianDesktop (集成系统底层交互)
...
├── GuardianDesktop/              # GuardianDesktop 桌面端项目 (主进程逻辑、打包配置)
├── sh/                    # 运维管理脚本 (启动、停止、回滚)
...
*   **📋 每日任务系统**: 侧边栏实时展示今日饮水、拉伸、专注等目标进度，目标达成触发额外奖励与庆祝。
*   **🐟 宠物经济系统**: 喂水/拉伸/专注获得小鱼干币，猫咪商店解锁多品种橙猫、蓝猫、布偶等。
*   **🐾 宠物深度互动**: 宠物拥有心情/饥饿/精力值，基于真实行为同步衰减，支持摸摸头、右键详情等互动。
*   **🏆 全能成就系统**: 包含隐藏成就与日常任务挑战，每解锁新成就自动获得金币礼包。
*   **📅 热力行为日历**: 仿 GitHub 风格的热力图，清晰展示过去 90 天的健康打卡强度。
*   **📅 日程结束提醒**: 设置会议结束时间，到时自动弹出拉伸引导，消除长会后的久坐伤害。
*   **🐟 小鱼干币经济系统**: 完成番茄钟获得小鱼干币，在猫咪商店解锁 9 种猫咪品种，`buy/unlock/coin.wav` 音效反馈。
*   **🐟 小鱼干币收支日志**: 完整记录每笔小鱼干币的来源（番茄钟/补水/拉伸/呼吸训练/成就）和去向（购买猫咪），含每日上限防刷机制，点击宠物界面鱼干数查看明细。
*   **🛒 猫咪领养商店**: 消耗小鱼干币解锁布偶猫/无毛猫等稀有品种，右键宠物弹窗进入商店。
*   **🐾 三维健康养成**: 饥饿度（按喝水提醒间隔实时衰减）、精力值（按久坐提醒间隔实时衰减）、清洁度（坏坐姿降低）实时反映在宠物状态面板，打卡后立刻回满。
*   **🏝️ 宠物精灵悬浮窗**: 灵动岛升级为猫咪精灵，悬停展开数据面板，左键呼出主窗口，右键撸猫互动。
...
- [x] **桌面级突破 (GuardianDesktop)**: 封装为跨平台桌面 App，支持系统托盘及全局快捷键。
...
- [x] **V4.0 — GuardianDesktop 同步增强**: 托盘菜单新增「账号同步」和「赏作者咖啡」入口，桌面端自动显示跨端同步引导条。
- [x] **V4.0 — 多主题兼容**: 新增 forest/ocean 专项 CSS 覆盖规则，全主题弹窗/输入框/按钮完美渲染。
- [x] **V4.0 — 🏝️ 健康灵动岛**: GuardianDesktop 悬浮窗（`Ctrl+Shift+W`），实时显示 Streak、番茄钟倒计时（localStorage 桥接）、CV 坐姿状态（探颈时变红），双击呼出主窗口。
...
- [x] **V4.2 — 🌟 互动猫咪桌宠 (Sprite Art Pet)**: 废弃文本 Emoji 素材，升级为纯粹的 64x64 骨骼序列帧像素猫咪。内建 4x4 精灵图调度引擎（支持 Idle/Walk/Play/Sleep 四态无缝动画）。支持 `meow/eat` 音效反馈、左键抚摸打滚交互与坐姿监测（不良坐姿自动进入低迷睡眠）。
- [x] **V4.3 — 🐟 小鱼干币经济系统**: 番茄钟发放小鱼干币，猫咪商店解锁 9 种品种，`buy/unlock/coin.wav` 音效。
- [x] **V4.3 — 🐾 三维健康养成**: 饥饿度/精力值/清洁度三维状态条，坏坐姿降低清洁度。
- [x] **V4.3 — 🏝️ 宠物精灵接管灵动岛**: widget.html 重构为猫咪精灵悬浮窗，悬停展开数据，左键呼出主窗口，右键撸猫。
- [x] **V4.5 — 🐟 小鱼干币收支日志（CoinLedger）**: 新增 `t_coin_log` 收支明细表，幂等防重 + 每日上限（番茄钟 60/补水 16/拉伸 9/呼吸 10/成就 30），修复启动重复推送成就消息 bug，宠物界面点击鱼干数查看收支明细。
